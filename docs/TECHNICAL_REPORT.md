...
`btc_backtest_cross_nf.py`, 291 features) — multi-asset pooling, additional feature families
(including an **index-futures macro-regime** family from NQ/ES + gold), per-direction barriers,
and a temporal-conv tokenizer, all adopted under a seed-controlled empirical protocol. Section **10** documents the **meta-labeling (M2) sizing layer** — a
...
- `crypto_data.py` refreshes Binance klines, drops the still-forming candle, and **merges into the
...
### 9.2 Feature extensions (256 → 291)
...
- **Macro-regime (index futures + gold, 9 features):** the risk-on/off environment BTC couples
  to, from **daily** Nasdaq-100 (NQ) & S&P-500 (ES) futures (`yfinance`) + gold (Binance PAXG):
  per-asset rolling **BTC↔NQ β / correlation** and **BTC↔gold β**, their interaction
  `mac_regime = β_NQ − β_gold`, NQ realised-vol / 5-day momentum, the **NQ−ES growth-rotation
  spread**, and a `mac_stale` / `mac_closed` pair encoding the 23/5-vs-24/7 gap. The **only**
  feature family sourced outside the crypto pairs → a genuinely orthogonal signal (BTC↔NQ
  corr ≈ 0.30). Returns/betas only (stationary); raw index levels are never fed in.

> **Causal daily→4h alignment (no look-ahead).** Index futures trade 23/5 with weekend/holiday
> gaps, so `add_macro()` (1) exposes the daily macro bar for date *D* only from *D+1 00:00 UTC*
> (a `+1-day` shift before the forward-fill), so a 4h crypto bar never sees a same-day macro
> close; (2) computes betas on macro **trading days** (weekdays) and freezes them over weekends,
> avoiding a double-counted Friday return on crypto's 24/7 grid; (3) neutral-fills warm-up /
> pre-gold-listing (< 2020-08) rows with the `mac_closed` flag set. This was the correctness core
> of the feature — an earlier version computed betas on the 24/7 grid and silently updated them
> on Sat/Sun (stale-data double-count), fixed by the trading-day construction.
...
| **NQ/ES + gold macro-regime** | data + feature | **adopted** — 10-seed A/B: active acc up 7/10 (+0.51pp) **and drawdown better 6/10**; the only candidate all-project to improve drawdown |
...
| Vol-scaled ATR barriers | label | rejected — 5/5 seeds worse (calm-regime barriers shrink → resolve on microstructure noise) |
| `seq_len` 64 → 84 | lookback | rejected — 5-seed wash; useful memory saturates by ~11 days |
| Perp funding-rate + basis | feature | rejected — 10-seed A/B: acc 5/10 −0.33pp, PF flat, drawdown coin-flip |

**Recurring lesson:** the highest-EV lever is a new orthogonal **data scope** — multi-asset
pooling first, then the index-futures macro-regime. Incremental single-feature and knob-tweak
ideas (barriers, lookback, extra cross-asset transforms) are saturated at a robust local optimum,
and *heavy* architecture changes don't help — except a *minimal* tokenizer upgrade that preserves
the working transformer. The refined nuance (macro **adopted** vs funding/basis **rejected**, both
new orthogonal sources): new data wins as a **slow regime reframe** (weeks-scale risk-on/off),
not a **fast positioning signal** that overlaps the existing order-flow features and mismatches
the 16h trade horizon.
...
- **13 curated features (not all 291):** M1 conviction (P, signed conviction, direction) +
...
**PCA absorption ratio** · **anchored VWAP** · **permutation (path) entropy** ·
**index-futures macro-regime (NQ/ES + gold, causal daily→4h forward-fill, 23/5-aware)** · ATR /
Wilder EMA · order-flow imbalance · VPIN · Amihud illiquidity · realised bipower variation (jump
