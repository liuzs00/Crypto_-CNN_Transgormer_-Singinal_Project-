# Feature Dictionary — Production Model (`emb_cross_nf`)

Model: `btc_eth_sol_pooled_model_emb_cross_nf.pth` · Trainer: `New_CNN_Transformer_Train.py`
**291 features** per candle, all **scale-invariant** (ratios / returns / z-scores) and
strictly **causal** (no look-ahead).

## Structure (how 291 is composed)
...
| **Macro-regime** (NQ/ES index futures + gold) | 9 | `add_macro()` |
| **Total** | **291** | |
...
## 13. Macro-Regime Features (NQ/ES index futures + gold) — **new family**
The risk-on/off environment crypto couples to, sourced from **daily** Nasdaq-100 (NQ) & S&P-500
(ES) futures (via `yfinance`, `macro_data.py`) and gold (Binance PAXG), forward-filled onto the
4h grid by `add_macro()`. This is the only feature family sourced **outside** the crypto pairs —
a genuinely orthogonal information source (BTC↔NQ correlation ≈ 0.30). Betas/correlations use
returns only (stationary); raw index levels are never fed in.

| Feature | Scope | Formula | Meaning |
|---|---|---|---|
| `mac_nq_beta` | per-asset | 60-trading-day cov(asset_r, NQ_r) / var(NQ_r) | leverage to the equity risk-on/off leg (β to Nasdaq) |
| `mac_nq_corr` | per-asset | 60-trading-day corr(asset_r, NQ_r) | strength of the equity coupling |
| `mac_gld_beta` | per-asset | 60-trading-day cov(asset_r, gold_r) / var(gold_r) | β to gold (safe-haven / debasement leg) |
| `mac_regime` | per-asset | `mac_nq_beta − mac_gld_beta` | **regime interaction** — high ⇒ trading as leveraged Nasdaq while decoupled from gold; low ⇒ risk-off / haven behaviour |
| `mac_nq_vol` | market | 20-day std of NQ daily returns | equity-vol regime (VIX proxy from realised NQ vol) |
| `mac_nq_ret5` | market | NQ 5-day return | macro momentum / risk appetite |
| `mac_esnq_spread` | market | NQ 5d return − ES 5d return | growth-vs-broad rotation (Nasdaq leading/lagging the S&P) |
| `mac_stale` | market | days since the last fresh macro bar, clipped [0,5] | **23/5 staleness** — how old the forward-filled macro leg is (weekends/holidays) |
| `mac_closed` | market | `1` if `mac_stale > 1.4` else `0` | macro market-closed flag (index futures are 23/5, crypto 24/7) |

> **Strict causality (no look-ahead).** Index futures are 23/5 with weekend/holiday gaps, so:
> (1) the daily macro bar for date *D* is exposed only from *D+1 00:00 UTC* (a `+1-day` shift
> before the forward-fill), so a 4h crypto bar never sees a same-day macro close; (2) betas are
> computed on macro **trading days** (weekdays) and then frozen over weekends, avoiding a
> double-counted Friday return; (3) warm-up / pre-listing rows (gold < 2020-08) are neutral-filled
> `0` with the `mac_closed` flag set. Verified: `mac_stale` = 2.44 (Mon) / 0.44 (Sat) / 1.44 (Sun),
> beta frozen Fri→Mon.
...
- **Inference dependency:** because of the cross-asset + absorption features, BTC inference
  with this model requires **live ETH and SOL data** (BTC-only inference is not possible); the
  macro family additionally needs **daily NQ/ES (yfinance) + gold**, but degrades gracefully to
  a neutral/last-known macro leg (with `mac_closed` set) if that source is unavailable.
...
- **Path entropy** (`pent`, `pent_chg`) cleared a **3-seed paired A/B** — active-signal accuracy
  improved in 3 of 3 seeds (~+1.3pp mean) with val-loss better on average — unlike several
  rejected ideas (Hawkes self-excitation, wavelet detail coefficients) that did not replicate
  across seeds. Modest but robust, and **orthogonal**: it encodes path *predictability*, which the
  EMA cascade and realized-vol features do not.
- **Macro-regime** (`mac_*`, §13) is the **most recent adoption** and the single feature family
  to clear a full **10-seed** A/B: active-signal accuracy up 7/10 seeds (+0.51pp) **and drawdown
  better 6/10** — the only candidate all-project to improve drawdown, from a genuinely new
  orthogonal data *scope* rather than another transform of the crypto tape. (Later rejects that
  did *not* clear the bar: gold ratios/β, Rényi transfer-entropy, Lévy-area path signature, and
  perp funding-rate + basis — orthogonal data helps only as a *slow regime reframe*, not a fast
  positioning signal that overlaps the order-flow features.)
