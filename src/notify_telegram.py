...
from paths import OUTPUTS_DIR, CONFIG_DIR
SIG_CSV = os.path.join(OUTPUTS_DIR, 'btc_predict_cross_nf_signals.csv')


def get_config():
    tok = os.environ.get('TELEGRAM_BOT_TOKEN'); chat = os.environ.get('TELEGRAM_CHAT_ID')
    cfg = os.path.join(CONFIG_DIR, 'telegram_config.json')
...
def build_message():
    ...
    # entry time = this 4h bar's CLOSE (open + 4h) — the moment the signal is acted on
    entry_t = (pd.Timestamp(r['timestamp']) + pd.Timedelta(hours=4)).strftime('%Y-%m-%d %H:%M:%S')
    return (f"📊 BTC Next Signal — {icon}\n"
            f"Time: {entry_t} UTC0\n"
            f"Enter: {r['close']:,.0f}\n"
            f"M1 conf: {r['prob_long']:.3f}\n"
            f"Regime: {r['vol_regime']}\n"
            f"M2 conf: {conf_s}\n"
            f"Size: {size_s}")
