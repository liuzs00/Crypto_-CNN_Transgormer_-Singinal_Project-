...
from paths import DATA_DIR, MODELS_DIR, OUTPUTS_DIR
SEQ_LEN      = 64
...
# 12. SAVE
# ─────────────────────────────────────────────
ckpt = os.path.join(MODELS_DIR, 'btc_lstm_model.pth')
torch.save({
...
plt.tight_layout()
curves = os.path.join(OUTPUTS_DIR, 'training_curves.png')
...
sig_df.to_csv(os.path.join(OUTPUTS_DIR, 'btc_signals.csv'), index=False)
active = sig_df[sig_df['signal'] != 'NEUTRAL']
active.to_csv(os.path.join(OUTPUTS_DIR, 'btc_signals_active.csv'), index=False)
...
plt.tight_layout()
attn_path = os.path.join(OUTPUTS_DIR, 'attention_analysis.png')
