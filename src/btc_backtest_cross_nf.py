...
from paths import MODELS_DIR, OUTPUTS_DIR, SRC_DIR
DATA_DIR  = FE.DATA_DIR             # alias (btc_predict_cross_nf.py reads BT.DATA_DIR / BT.ASSETS)
ASSETS    = FE.ASSETS
CKPT_PATH = os.environ.get('CKPT', os.path.join(MODELS_DIR, 'btc_eth_sol_pooled_model_emb_cross_nf.pth'))
...
def build():
    ...
    if 'mac_regime' in feat_cols: bases = FE.add_macro(bases)   # auto-detect macro (NQ/ES) model
    if 'fund_rate' in feat_cols:  bases = FE.add_funding(bases)  # auto-detect funding/basis model
...
def main():
    ...
    if len(eq) > 1:
        ...
        out = os.path.join(OUTPUTS_DIR, f'backtest_cross_nf_{args.split}.png'); plt.savefig(out, dpi=150); plt.close(); print(f"  equity plot -> {out}")
    trades.to_csv(os.path.join(OUTPUTS_DIR, f'backtest_cross_nf_trades_{args.split}.csv'), index=False)
